...
class DbOracle extends Db{
    ...
    private     $table        = '';
    protected   $selectSql    = 'SELECT * FROM (SELECT DBUSRINWX.*, rownum AS numrow FROM (SELECT  %DISTINCT% %FIELD% FROM DBUSRINWX.%TABLE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%) DBUSRINWX ) %LIMIT%';
