...
class backupAction extends backendAction
{
    ...
    public function import()
    {
        ...
        if ($this->_import_vol($backup['file']))
        {
            ...
            {
                $vol++;
                $link = 'index.php?g=adminn&m=' . MODULE_NAME . '&a=import&vol=' . $vol .
                    '&backup=' . urlencode($this->backup_name);
                $this->success(sprintf(L('bakup_import_succ'), $vol - 1), $link);
            } else
            {
                $this->success(L('restore_success'), 'index.php?g=adminn&m=' . MODULE_NAME .
                    '&a=restore');
            }
        }
    }
    ...
    private function _deal_result($tables, $vol, $sizelimit)
    {
        ...
        if (empty($tables))
        {
            ...
            $this->success(L('bakup_succ'), 'index.php?g=adminn&m=' . MODULE_NAME .
                '&a=restore');
        } else
        {
            //开始下一卷
            $this->_set_vol($vol); //设置分卷记录
            $link = 'index.php?g=adminn&m=' . MODULE_NAME .
