...
class custom_menuAction extends backendAction {
	...
	public function index_() {
		...
		$big_menu = array ('title' => '添加菜单', 'iframe' => U ( 'custom_menu/add' ), 'id' => 'add', 'width' => '520', 'height' => '360' );
	...
	public function tag_add() {
		if (IS_POST) {
			...
			foreach ( $tag_ids_arr as $val ) {
				$cate_tag_mod->add ( array ('cate_id' => $cate_id, 'tag_id' => $val ) );
			}
	...
	public function ajax_upload_img() {
		...
		if (! empty ( $_FILES ['img'] ['name'] )) {
			$result = $this->_upload ( $_FILES ['img'], 'item_cate', array ('width' => C ( 'pin_itemcate_img.width' ), 'height' => C ( 'pin_itemcate_img.height' ), 'remove_origin' => true ) );
			if ($result ['error']) {
	...
	public function index() {
		...
		$result = $this->_mod->where($where)->order ( $sort . ' ' . $order )->select ();
		foreach ($result as $key=>$value) {
			$childMenu = $this->_mod->where('pid=' .$value['id'] )->order ( $sort . ' ' . $order )->select ();
			$value['childMenu'] = $childMenu;
			$list[] = $value;
		}
		$this->assign('list', $list);
		$this->display();
	...
	public function add() {
		if (IS_POST) {
			...
			$menu_data['name'] = $name;
			$menu_data['status'] = 1;
			$menu_data['uid'] = $this->shopId();
			$parentId = $_POST['pid'];
			if (!empty($parentId)) { //添加二级菜单
				$menu_data['pid'] = $parentId;
			} 
			$menu_data['type'] = $type;
			if ($type=='0') { //点击
				$media_id = $_POST['media_id'];
				$keyword = M('keyword')->field('kid,kyword')->where('media_id = ' . $media_id)->find();
				$menu_data['media_id'] =$media_id;
				$menu_data['content'] = $keyword['kyword'];
				//dump($menu_data);
				//M('custom_menu')->data($menu_data)->add();
			
			} else {  //linkurl
				$linkurl = $_POST['linkurl'];
				$menu_data['content'] = $linkurl;
			}
			...
			if (empty($id)) { //添加一级菜单
				//判断一级菜单的数量
				$parentMenuCount = M('custom_menu')->where($where)->count();
				if ($parentMenuCount >= 3) {
					//$this->
					$canAdd = false;
					$msg = '一级菜单最多为三个';
				} else {
				...
				$childMenuCount = M('custom_menu')->where('pid='. $id)->count();
				if ($childMenuCount >= 5) {
					$canAdd = false;
					$msg = '二级菜单最多为五个';
				} else {
			...
			if ($canAdd) {
				...
				if (IS_AJAX) {
					$response = $this->fetch();
					$this->ajaxReturn(1, $msg, $response);
				} else {
					$this->display();
				}
			} else {
				...
				if (IS_AJAX) {
					$response = $this->fetch();
					$this->ajaxReturn(0, $msg, $response);
				} else {
					$this->display();
				}
			}
	...
	public function edit(){
		if(IS_POST) {
			...
			$this->_mod->data($menu_data)->save();
			
			IS_AJAX && $this->ajaxReturn(1, L('operation_success'), '', 'add');
			$this->success(L('operation_success'));
		} else {
			$id=$_GET['id'];
			$info = M('custom_menu')->find($id);
	...
	public function public_menu() {
			$data = $this->queryMenu ();
			$accountid = $this->shopId();
			$data = preg_replace_callback ( '/\\\\u([0-9a-f]{4})/i',
					create_function ( '$matches', 'return mb_convert_encoding(pack("H*", $matches[1]), "UTF-8", "UCS-2BE");' ), $data);
			$msg = publicMenu($accountid,  $data );//$this->curl_menu ( $access_token, preg_replace ( "#\\\u([0-9a-f]+)#ie", "iconv('UCS-2', 'UTF-8', pack('H4', '\\1'))", $data ) );
			 if ($msg ['errmsg'] == 'ok') {
				$this->success ( '创建自定义菜单成功!' );
			} else {
				$this->error ( '创建自定义菜单失败!' );
	...
	public function queryMenu() {
		...
		$topmemu = $this->_mod->field ( 'id,name,type,content' )->where ( $where )->order ( 'id asc' )->select ();
		foreach ( $topmemu as $key ) {
			$nextmenu = $this->_mod->field ( 'id,name,type,content' )->where ( 'pid=' . $key ['id'] . ' and status=1' )->order ( 'id asc' )->select ();
			if (count ( $nextmenu ) != 0){ //有下级栏目
				foreach ( $nextmenu as $key2 ) {
					if ( $key2 ['type'] == 0 ){ //click
						$kk [] = array ('type' => 'click', 'name' => $key2 ['name'], 'key' => $key2 ['content'] );
					} else {  //view
						$kk [] = array ('type' => 'view', 'name' => $key2 ['name'], 'url' => $key2 ['content'] );
					}
				}
				$keyword ['button'] [] = array ('name' => $key ['name'], 'sub_button' => $kk );
				$kk = '';
				...
				if ($key ['type'] ==0 ){ //click
					$keyword ['button'] [] = array ('type' => 'click', 'name' => $key ['name'], 'key' => $key ['content'] );
				} else  { //view
					$keyword ['button'] [] = array ('type' => 'view', 'name' => $key ['name'], 'url' => $key ['content'] );
				}
