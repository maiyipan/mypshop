...
class item_cateAction extends backendAction {
	...
	public function index() {
        ...
        $result = $this->_mod->order($sort . ' ' . $order)->select(); //where($where)->
        $array = array();
        foreach($result as $r) {
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog btn btn-link" data-uri="'.U('item_cate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_item_cate').'" data-id="add" data-width="600" data-height="360">'.L('add_item_subcate').'</a> 
                                <a href="javascript:;" class="J_showdialog btn btn-link" data-uri="'.U('item_cate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="520" data-height="360">'.L('edit').'</a> 
                                <a href="javascript:;" class="J_confirmurl btn btn-link" data-acttype="ajax" data-uri="'.U('item_cate/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>' ;
//             					<a href="javascript:;" class="J_showdialog btn btn-link" data-id="edit" data-uri="'.U('item_cate/pre', array('id'=>$r['id'])).'" data-title="'.L('edit').' - '.$r['name'].'"  data-width="400" data-height="130">预览</a>';
    ...
    public  function item_qrcode(){
    	...
    	$urlfrist="/item/index/cate_id/".$id."/sid/".$shopid;//最低成商品分类url
    	$urlsecond="/index/sortSecond/id/".$id."/sid/".$shopid;//商品分类一级
    	...
    	if($data['qrcode']!=null){
    		...
    		$data=$qrcode->qrcode($url,$id,$table);
    		return $data;
    	}
    }
    /**
