...
class baseAction extends Action
{
    protected function _initialize() {
        ...
        if (false === $section = F('section')) {
        	$section = D('section') -> section_cache();
        }
        C($section);
    ...
    public function getShop() {
    	...
    	//查询id下是否有对应的商城
    	//有则返回所有的，没有则返回自己
    	$shop = M('shop') -> where(array('uid'=>$tmpsid,'status'=>1))->find();
    	...
    	if ($shopCout > 0) {  //父
    		...
    		$shopList = M('shop')->where('pid = ' . $shop['pid'])->select(); //相同父id
    		foreach ($shopList as $key=>$val) {
    			$uid = $uid . ',' .$val['uid'];
    		}
    	}
