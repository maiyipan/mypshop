...
class passport
{
    ...
    public function __construct($name) {
        $file = LIB_PATH . 'Ilzlib/passport/' . $name . '.php';
        include_once  $file;
    ...
    static function uc($name) {
        include LIB_PATH . 'Ilzlib/passport/' . $name . '.php';
        $class = $name . '_passport';
        return new $class();
    }
     ...
     * 注册新用户
     */
    public function register($username, $password, $email, $gender, $headimgurl) {
        if (!$add_data = $this->_us->register($username, $password, $email, $gender, $headimgurl)) {
