...
class user_visitor {
	...
	public function __construct() {
		...
		*/
		...
		if (isset ( $_GET ['key'] )) {
			$user = M ( 'user' )->field ( 'id,username,headimgurl,score,balance' )->where ( "username='" . trim ( $_GET ['key'] ) . "'" )->find ();
			if ($user) {
				$data ['last_time'] = time ();
		...
		if (session ( '?user_info' )) {
			...
			$user_info = M ( 'user' )->field ( 'id,username,headimgurl,score,balance' )->where ( array ('id' => $user_info ['id'], 'password' => $user_info ['password'] ) )->find ();
			if ($user_info) {
				//记住登陆状态
		...
		/* if(empty($_SESSION ['user_info'])){
			$user = M ( 'user' )->where ( array ('id' => 100 ) )->find();
			$_SESSION ['user_info'] = $user;
		} */
	}
	...
	public function login($uid, $remember = null) {
		...
		$user_info = $user_mod->field ( 'id,username,password,score,headimgurl')->find ( $uid );
		//保持状态
		$this->assign_info ( $user_info );
