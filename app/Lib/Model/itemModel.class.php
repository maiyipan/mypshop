...
//         $item['imgs'][] = $item['img'];
        ...
        M('item_base')->create($item);
        $baseid=  M('item_base')->add();
    ...
    public function update_comments($id, $comment = '') {
        ...
        if (is_array($comment)) {
            ...
            //刷新
            $comments_cache = M('item_comment')->field('id,userid,username,info,images')->where(array('item_id'=>$id, 'status'=>'1'))->order('id DESC')->limit($max_num)->select();
        }
    ...
    public function update_tag_cache($id, $tag_cache = '') {
        if (!is_array($tag_cache)) {
            ...
            Log::write(M('item_tag')->getLastSql());
            $tag_res = M('tag')->where(array('id'=>array('IN', $tag_ids)))->select();
             Log::write(M('tag')->getLastSql());
            $tag_cache = array();
