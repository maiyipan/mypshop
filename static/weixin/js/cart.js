...
function cancleSelect(id){
	var subtotal_span = $('#item' + id + '_subtotal');
	var amount_span = $('#cart_amount');
	var item = $('#cancle_' + id);
	var isChecked = item.hasClass('checked');
	if (isChecked) {
		$('#cancle_' + id).removeClass("checked");
		$('#cancle_' + id).addClass('uncheck');
		
	} else {
		$('#cancle_' + id).addClass('checked');
		$('#cancle_' + id).removeClass("uncheck");
	}

	$.post("index.php?m=shopcart&a=cancle_item", {
		itemId : id
	}, function(data) {
		if (data.status == 1) {
			//subtotal_span.html(price_format(data.item.price * data.item.num));
